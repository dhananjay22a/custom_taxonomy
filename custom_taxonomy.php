	...
	function widget_css() { 
		?> 
     		<style type="text/css"> 
				...
				#pmc_customtaxonomy_type_main_div .news_item_title { width:183px; height:37px; padding:0 5px; float:left; }
	...
	*/
	class taxonomy_widget extends \WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			$news = get_transient('custom_taxonomy_posts_key');
			if ($news === false) {
				$news = new \WP_Query( $args );
				//Adding data to cache
				set_transient('custom_taxonomy_posts_key', $news, 3600 * 24);
			}
				...
				while ( $news->have_posts() ) : $news->the_post();
					...
					echo '<div style="color:'.$textColor.'; background:'.$bgColor.';" class="news_item_main_div">';
						...
						echo '<div style="color:'.$textColor.'; background:'.$bgColor.';" class="news_item_title" >';
							?>
								<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php echo $this->short_title(41); ?></a><br />
							<?php 
						echo '</div>';
